...
- **CSS SCSS @for bug** — `globals.css` line 328 had a SCSS `@for` loop that doesn't compile in standard CSS. Replaced with explicit CSS rules. This was silently breaking Tailwind v4 theme compilation.
- **Animation preset system** — Created `src/lib/animations.ts` with shared presets (`fadeUp`, `slideDown`, `staggerMedium`, `staggerItem`, `springGentle`, `springSnap`, `zoomIn`). Converted 15+ files from inline `initial/animate` variants to shared imports. Added `will-change: transform, opacity` GPU hints.
- **Skeleton class unification** — Replaced `animate-pulse` with proper `.skeleton` CSS class using `bg-surface-raised` token in 3 components. Could not remove `animate-pulse` from tailwind safelist because it's used as a `className` string in `page.tsx` badge/status logic.
- **Empty state improvements** — AnomalyDashboard, KanbanWorkflow, ReportViewer: replaced "fetching..." text with proper skeleton states. Added "No anomalies found" / "No reports yet" empty states with consistent styling.
- **Landing page overhaul (ultra-max)** — Three.js 3D hero with floating receipt card + orbiting particles. TiltCard component with pointer-reactive 3D rotation + glare effect. AnimatedCounter with scroll-triggered number rollup. 28-feature data model (`feature-content.ts`). `/features` grid page with search/filter. `/features/[slug]` dynamic feature detail page. 3D hero section, animated counters, 3D tilt cards, particle effects, ultra-max motion animations.
- **ESLint CI fix** — Fixed 3 `react-hooks/set-state-in-effect` errors (Scene3D `useIsMobile` → `useState` lazy initializer, `useScanLineTexture` → `useMemo`, TiltCard `isTouch` → render-time constant). Removed ~8 unused imports across 5 files. CI pipeline now passes: 0 errors, 0 warnings.
