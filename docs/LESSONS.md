...
## Production: CSP blocks all inline styles/scripts, preventing hydration — Cause: `proxy.ts` set `Content-Security-Policy` header with `'nonce-<random>'` but Next.js 16 Turbopack has a nonce-propagation bug — inline `<script>` and `<style>` blocks lack nonce attributes. CSP blocked ALL inline content, so RSC payload injection scripts, hydration scripts, and inline styles never executed. The server-rendered HTML (FullPageLoader) stayed visible forever. — Fix: Added `'unsafe-inline'` to both `style-src` and `script-src` CSP directives. Also added `x-nonce` response header for future nonce support when Turbopack fixes the bug. Remove `'unsafe-inline'` once upstream nonce propagation works.
