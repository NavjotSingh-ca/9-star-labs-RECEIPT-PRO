...
## Production: CSP blocks all inline styles/scripts, preventing hydration (attempt 1) — Cause: `proxy.ts` set `Content-Security-Policy` header with `'nonce-<random>'` but Next.js 16 Turbopack has a nonce-propagation bug — inline `<script>` and `<style>` blocks lack nonce attributes. CSP blocked ALL inline content. — Fix attempt 1: Added `'unsafe-inline'` alongside `'nonce-...'`. This FAILED because per CSP spec, `'unsafe-inline'` is silently IGNORED when any `'nonce-'` or hash is present in the same directive (https://w3c.github.io/webappsec-csp/#directive-script-src). — Fix (final): Removed nonce from the CSP directives entirely. Using `'unsafe-inline'` alone for both `style-src` and `script-src` until Turbopack nonce propagation is fixed. Also added `manifest-src 'self'` to prevent Vercel SSO manifest fetch violations. x-nonce header retained for future use.
