...
import { ArrowLeft, ShieldCheck, List, Lock, FileText, Calendar } from 'lucide-react';
...
function Section({ id, title, children }: { id: string; title: string; children: React.ReactNode }) {
  return (
    <section id={id} className="scroll-mt-24 animate-in fade-in slide-up-from-bottom-4 duration-700">
      <h2 className="mt-12 mb-5 text-xl font-bold text-text-primary border-b border-glass-border pb-3.5">{title}</h2>
      <div className="space-y-4 text-[0.9375rem] leading-7 sm:text-sm sm:leading-7 text-text-secondary">{children}</div>
    </section>
  );
}
    ...
    <div className="min-h-screen bg-champagne">
      <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <aside className="hidden lg:block lg:col-span-4 lg:sticky lg:top-24 lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto lg:pr-2">
            <div className="rounded-[2.5rem] border border-glass-border bg-gradient-to-br from-champagne/[0.03] via-surface/50 to-transparent p-6 backdrop-blur-sm">
              <div className="flex items-center gap-2 mb-4">
                <List className="h-4 w-4 text-champagne" />
                <p className="text-xs font-black uppercase tracking-widest text-champagne">Contents</p>
              </div>
              <ol className="space-y-1.5">
                {TOC.map(({ id, label }) => (
                  <li key={id}>
                    <a
                      href={`#${id}`}
                      className="block text-sm text-text-secondary hover:text-champagne transition-all py-1.5 px-3 rounded-xl hover:bg-champagne/[0.04]"
                    >
                      {label}
                    </a>
                  </li>
          ...
          <div className="lg:col-span-8">
            <div className="rounded-[2.5rem] border border-glass-border bg-card p-8 sm:p-14">
              ...
              <div className="mb-10 flex items-start gap-5 border-b border-glass-border pb-10">
                <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-xl bg-champagne/15 shadow-[0_0_24px_-4px_rgba(139,115,85,0.25)]">
                  <ShieldCheck className="h-8 w-8 text-champagne" />
                </div>
                <div>
                  <h1 className="text-3xl font-black tracking-tight text-text-primary sm:text-4xl bg-gradient-to-r from-champagne to-obsidian bg-clip-text text-transparent">
                    Privacy Policy
                  </h1>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2.5">
                    <span className="inline-flex items-center gap-1.5 rounded-full border border-champagne/30 bg-champagne/[0.08] px-3.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-champagne transition-all hover:border-champagne/40 hover:bg-champagne/[0.12]">
                    ...
                    <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-success/30 bg-emerald-success/[0.08] px-3.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-emerald-light transition-all hover:border-emerald-success/40 hover:bg-emerald-success/[0.12]">
              ...
              {/* Mobile TOC - below header on mobile/tablet */}
              <nav className="mb-10 lg:hidden">
                <div className="rounded-[2.5rem] border border-glass-border bg-gradient-to-br from-champagne/[0.03] via-surface/50 to-transparent p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <List className="h-3.5 w-3.5 text-champagne" />
                    <p className="text-xs font-black uppercase tracking-widest text-champagne">Contents</p>
                  </div>
                  <ol className="space-y-1.5">
                    {TOC.map(({ id, label }) => (
                      <li key={id}>
                        <a
                          href={`#${id}`}
                          className="block text-sm text-text-secondary hover:text-champagne transition-all py-1"
                        >
                          {label}
                        </a>
                      </li>
              ...
              {/* Opening Statement */}
              <div className="mb-8 rounded-[2rem] border border-champagne/20 bg-gradient-to-br from-champagne/[0.04] via-card/80 to-transparent p-5 animate-in fade-in slide-up-from-bottom-4 duration-700">
              ...
              <Section id="information-collected" title="2. Personal Information We Collect">
                ...
                <div className="rounded-[2rem] border border-glass-border bg-surface/30 p-4 space-y-4 stagger-enter">
                  {[
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-300">
              ...
              <Section id="purposes" title="3. Purposes of Collection & Use">
                ...
                <ol className="list-decimal list-outside ml-5 space-y-3 stagger-enter">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-500"><B>We do not sell your personal information.</B> We do not use your financial data for advertising.</p>
              </Section>

              {/* Section 4 */}
              <Section id="ai-ml" title="4. AI & Machine Learning Disclosure">
                <div className="rounded-[2rem] border border-warning/20 bg-warning/[0.05] p-4 mb-4 animate-in fade-in slide-up-from-bottom-4 duration-700">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-100"><B>What is transmitted to Google:</B> The base64-encoded image of your receipt or financial document, along with a structured prompt instructing the AI to extract specific financial fields. No other personal information (name, email, account ID) is transmitted alongside the image.</p>
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-150"><B>Google&apos;s data usage policy:</B> Under Google&apos;s enterprise API terms for Gemini API access, data processed through the API is <B>not used to train Google&apos;s public AI models</B>. Google acts as a data processor on our behalf. For full details, see <ExternalA href="https://ai.google.dev/gemini-api/terms" label="Google Generative AI Terms">Google&apos;s Generative AI Terms</ExternalA> and <ExternalA href="https://policies.google.com/privacy" label="Google Privacy Policy">Google&apos;s Privacy Policy</ExternalA>.</p>
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-200"><B>Vector embeddings for semantic search:</B> Text descriptions derived from your receipts (vendor name, category, notes, amount) are also transmitted to Google&apos;s text-embedding-004 model to generate numerical vector representations stored in our database. These vectors enable natural-language search but do not contain full receipt images or sensitive financial identifiers.</p>
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-250"><B>AI accuracy limitations:</B> AI extraction is subject to error. You are responsible for reviewing and verifying all AI-extracted data before submission to the CRA or any accounting system. CRA readiness scores are informational tools, not legal guarantees of deductibility. Always consult a qualified Canadian accountant or tax professional for tax advice.</p>
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-300"><B>Opt-out:</B> You may choose to manually enter receipt data without using the AI extraction feature. Contact <A href="mailto:9starlaba@gmail.com">9starlaba@gmail.com</A> to request a manual-entry-only account mode.</p>
              </Section>
              ...
              <Section id="storage-security" title="5. Data Storage & Security">
                ...
                <ul className="list-disc list-outside ml-5 space-y-2 stagger-enter">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-300">Despite these safeguards, no internet-based service can guarantee absolute security. If you discover a security vulnerability, please contact us immediately at <A href="mailto:9starlaba@gmail.com">9starlaba@gmail.com</A>.</p>
              </Section>

              {/* Section 6 */}
              <Section id="retention" title="6. Data Retention — 7-Year CRA Minimum">
                <div className="rounded-[2rem] border border-champagne/20 bg-champagne/[0.04] p-4 mb-2 animate-in fade-in slide-up-from-bottom-4 duration-700">
              ...
              <Section id="your-rights" title="7. Your Rights Under PIPEDA">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-600">To exercise any of these rights, submit a written request to <A href="mailto:9starlaba@gmail.com">9starlaba@gmail.com</A>. We will verify your identity before processing any access, correction, or deletion request.</p>
              </Section>
              ...
              <Section id="cookies" title="9. Cookies & Session Tokens">
                ...
                <div className="rounded-[2rem] border border-glass-border bg-surface/30 p-4 overflow-x-auto animate-in fade-in slide-up-from-bottom-4 duration-700">
              ...
              <Section id="third-party" title="10. Third-Party Service Providers">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-500">We do not sell, rent, or trade your personal information to any third party for their own purposes.</p>
              </Section>
              ...
              <Section id="cross-border" title="11. Cross-Border Data Transfers">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-200"><B>Our commitment to Canadian data residency:</B> We are actively evaluating Supabase&apos;s Canadian region hosting (AWS ca-central-1) and will migrate as soon as it becomes generally available on our service tier. We will notify all users by email prior to any change in data residency.</p>
              </Section>

              {/* Section 12 */}
              <Section id="quebec" title="12. Quebec Law 25 Compliance">
                <div className="rounded-[2rem] border border-champagne/20 bg-champagne/[0.04] p-4 mb-4 animate-in fade-in slide-up-from-bottom-4 duration-700">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-100">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-150">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-200">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-250">
                ...
                <p className="animate-in fade-in slide-up-from-bottom-4 duration-700 delay-350">
              ...
              <Section id="updates" title="13. Policy Updates & Contact">
                ...
                <ul className="list-disc list-outside ml-5 space-y-1 stagger-enter">
                ...
                <div className="mt-6 rounded-[3rem] border border-champagne/20 bg-champagne/[0.04] p-6 animate-in fade-in slide-up-from-bottom-4 duration-700 delay-100">
                ...
                <div className="mt-4 rounded-[2rem] border border-glass-border bg-surface/30 p-4 animate-in fade-in slide-up-from-bottom-4 duration-700 delay-200">
