...
const receiptFormSchema = z.object({
  // Core receipt fields - ALL required per ReceiptForm type
...
transaction_date: z.string().min(1, 'Date is required'),
  ...
  paid_by: z.enum(['company_card', 'employee_cash', ''] as const),
  reimbursement_status: z.enum(['pending', 'approved', 'rejected', ''] as const),
  approval_status: z.enum(['submitted', 'approved', 'rejected', ''] as const),
...
const setFieldError = (name: keyof ReceiptForm, message: string) => {
      form.setError(name, { type: 'manual', message });
    };

    const clearFieldError = (name: keyof ReceiptForm) => {
    ...
    const restoreFormState = (state: ReturnType<typeof getFormState>) => {
