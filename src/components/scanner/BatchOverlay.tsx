...
/**
 * Displays batch processing progress with error detection.
 * Uses ARIA progressbar role for screen reader announcements.
 */
export default function BatchOverlay({ progress, total, error, failedItems = 0 }: BatchOverlayProps) {
  ...
  return (
    <div
      className={`mb-4 overflow-hidden rounded-[3rem] border bg-obsidian shadow-[0_0_20px_rgba(190,169,142,0.1)] ${
        hasError ? 'border-danger/30' : 'border-champagne/30'
      }`}
      role="progressbar"
      aria-valuenow={progress}
      aria-valuemin={0}
      aria-valuemax={total}
      aria-valuetext={hasError ? error || `${failedItems} item(s) failed` : `${progress} of ${total} processed`}
      aria-live="polite"
    >
      <div className="relative px-5 py-3">
        <div className="flex items-center gap-3 relative z-10">
          {hasError ? (
            ...
            <Layers className="h-5 w-5 animate-pulse text-champagne" aria-hidden="true" />
          )}
          <div className="flex-1">
            <p className={`text-sm font-bold ${hasError ? 'text-danger' : 'text-champagne'}`}>
              {hasError ? 'Processing Issues Detected' : 'Batch Processing Active'}
            </p>
          ...
          {!hasError && <Loader2 className="h-4 w-4 animate-spin text-champagne" aria-hidden="true" />}
        </div>
        <div
